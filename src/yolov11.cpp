...
YOLOv11::YOLOv11() {
    ...
    yolov11.opt = ncnn::Option();

#if NCNN_VULKAN
    yolov11.opt.use_vulkan_compute = use_gpu;
#endif

    yolov11.opt.num_threads = ncnn::get_big_cpu_count();
    yolov11.opt.blob_allocator = &blob_pool_allocator;
    yolov11.opt.workspace_allocator = &workspace_pool_allocator;

    yolov11.load_param(YOLOv11_PARAM_PATH);
    yolov11.load_model(YOLOv11_BIN_PATH);
}
...
int YOLOv11::detect(const cv::Mat& rgb, std::vector<Object>& objects) {
    int img_w = rgb.cols;
    int img_h = rgb.rows;

    // ultralytics/cfg/models/v8/YOLOv1111.yaml
...
int YOLOv11::draw(cv::Mat& rgb, const std::vector<Object>& objects) {
